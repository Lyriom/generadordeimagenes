...
function campaignBriefHtml(compact: boolean): string {
  ...
  return [
    ...
    '<p>PDF, AI, PSD, PPTX, artes, manuales, logos y tipografías se analizan como contexto. También acepta ZIP de fuentes. Un PDF sigue siendo un documento; sus páginas no se convierten en campañas ni KV separados.</p></div></div>',
    ...
    '<label class="dropzone source-drop" id="artwork-drop"><input id="artwork-files" type="file" multiple accept=".ai,.psd,.psb,.pdf,.pptx,.docx,.xlsx,.csv,.tsv,.txt,.rtf,.md,.png,.jpg,.jpeg,.webp,.bmp,.gif,.tif,.tiff,.avif,.ttf,.otf,.zip"', intakeDisabled, '>',
    '<span class="drop-icon" id="drop-icon">⇧</span><strong class="drop-title" id="drop-title">Arrastra todo el material aquí</strong>',
    '<span class="drop-hint" id="drop-hint">AI, PSD/PSB, PDF, PPTX, imágenes, logos, fuentes TTF/OTF o ZIP de fuentes</span></label>',
...
function activeCampaignHtml(): string {
  ...
  return [
    ...
    '<p>Cada archivo conserva su función como fuente de conocimiento. Sube logos y fondos limpios como PNG/JPG para fijarlos en todas las plantillas.</p></div><label class="ghost-button file-button">Añadir material<input id="add-source-files" type="file" multiple accept=".ai,.psd,.psb,.pdf,.pptx,.docx,.xlsx,.csv,.tsv,.txt,.rtf,.md,.png,.jpg,.jpeg,.webp,.bmp,.gif,.tif,.tiff,.avif,.ttf,.otf,.zip"></label></div>',
